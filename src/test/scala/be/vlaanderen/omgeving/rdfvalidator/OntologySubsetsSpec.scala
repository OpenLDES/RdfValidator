...
class OntologySubsetsSpec extends AnyFlatSpec with Matchers {
  ...
  "extractDisjointSubset" should "include owl:disjointWith statements between named resources" in {
    val ontology = parseTTL("""
      @prefix owl: <http://www.w3.org/2002/07/owl#> .
      @prefix ex:  <http://example.org/> .
      ex:Sensor owl:disjointWith ex:Actuator .
    """)
    val subset = OntologySubsets.extractDisjointSubset(ontology)
    subset.contains(null, OWL.disjointWith, null) shouldBe true
  }

  it should "exclude rdfs:subClassOf statements" in {
    val ontology = parseTTL("""
      @prefix owl:  <http://www.w3.org/2002/07/owl#> .
      @prefix rdfs: <http://www.w3.org/2000/01/rdf-schema#> .
      @prefix ex:   <http://example.org/> .
    ...
    """)
    val subset = OntologySubsets.extractDisjointSubset(ontology)
    subset.contains(null, RDFS.subClassOf, null)  shouldBe false
    subset.contains(null, OWL.disjointWith, null) shouldBe true
  }
}
