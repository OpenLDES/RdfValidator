...
import org.apache.jena.rdf.model.{Model, ModelFactory, RDFList, RDFNode, Resource, ResourceFactory}
...
class NigellaInferredModelSpec extends AnyFlatSpec with BeforeAndAfterAll with Matchers {
  ...
  private val OWL_MEMBERS = ResourceFactory.createProperty("http://www.w3.org/2002/07/owl#members")
  ...
  override def beforeAll(): Unit = {
    ...
    val disjointSubset = OntologySubsets.extractDisjointSubset(completeOntology)
  ...
  it should "have no class disjointness violations after full inference" in {
    val violations = checkDisjointness(inferredModel, completeOntology)
  ...
  private def checkDisjointness(dataModel: Model, ontology: Model): Seq[String] = {
    val disjointPairs = scala.collection.mutable.LinkedHashSet[(String, String)]()

    // owl:AllDisjointClasses with owl:members
    ontology.listSubjectsWithProperty(RDF.`type`, OWL2.AllDisjointClasses).asScala.foreach { allDisjoint =>
      val membersStmt = allDisjoint.getProperty(OWL_MEMBERS)
      if (membersStmt != null) {
        val members = membersStmt.getResource.as(classOf[RDFList]).asJavaList().asScala
          .filter(_.isResource).map(_.asResource())
          .filter(_.isURIResource).map(_.getURI)
          .toIndexedSeq
        for (i <- members.indices; j <- (i + 1) until members.length)
          disjointPairs += ((members(i), members(j)))
      }
    }

    // Pairwise owl:disjointWith
    ontology.listStatements(null, OWL.disjointWith, null.asInstanceOf[RDFNode]).asScala.foreach { stmt =>
      if (stmt.getSubject.isURIResource && stmt.getObject.isURIResource) {
        val a = stmt.getSubject.getURI
        val b = stmt.getObject.asResource().getURI
        val (lo, hi) = if (a < b) (a, b) else (b, a)
        disjointPairs += ((lo, hi))
      }
    }

    disjointPairs.toSeq.flatMap { case (uri1, uri2) =>
      val c1 = ontology.createResource(uri1)
      val c2 = ontology.createResource(uri2)
      val typed1 = dataModel.listSubjectsWithProperty(RDF.`type`, c1).asScala.toSet
      val typed2 = dataModel.listSubjectsWithProperty(RDF.`type`, c2).asScala.toSet
      (typed1 intersect typed2).map { r =>
        s"${if (r.isURIResource) r.getURI else r.toString} is typed as both ${c1.getLocalName} and ${c2.getLocalName} (disjoint)"
      }
    }
  }
}
