...
object RdfUtils {
  ...
  /**
   * Loads an OWL/RDF ontology from a Turtle file at the given path.
   *
   * @param path the filesystem path to the Turtle ontology file
   * @return a new [[Model]] populated with the ontology triples
    ...
    model.listStatements().asScala
      ...
      .foreach { case (uri, stmts) =>
        ...
        if (literals.isEmpty && uriObjects.nonEmpty && !hasBNodes) {
          ...
          val linkedTypes = uriObjects
            .flatMap(n => model.listStatements(n.asResource(), RDF.`type`, null).asScala
              .filter(_.getObject.isURIResource)
              .map(_.getObject.asResource().getURI))
            .distinct

          val propFrame = frame.putObject(uri)
          if (linkedTypes.size == 1) propFrame.put("@type", linkedTypes.head)
